...
quota_structure <- data.frame(
  cut = c(0.4, 0.3, 0.2, 0.1),
  payout_pct = c(0.07, 0.10, 0.13, 0.16),
  cut_sum = cumsum(c(0.4, 0.3, 0.2, 0.1)),
  amt = 225000 * c(0.4, 0.3, 0.2, 0.1),
  payout = 225000 * c(0.4, 0.3, 0.2, 0.1) * c(0.07, 0.10, 0.13, 0.16)
)

quota_structure$payout_sum <- cumsum(quota_structure$payout)
quota_structure$amt_sum <- cumsum(quota_structure$amt)

print(quota_structure)
...
calculate_earnings <- function(fraction_quota) {
  total_amt <- fraction_quota * 225000
  
  # Filter the rows where the cumulative amount is less than or equal to total_amt
  applicable_brackets <- quota_structure[quota_structure$amt_sum <= total_amt, ]
  
  # Sum the payouts from the applicable brackets
  payout <- sum(applicable_brackets$payout)
  
  # If there's a remaining amount in the last bracket
  if (total_amt > max(applicable_brackets$amt_sum)) {
    remaining_amt <- total_amt - max(applicable_brackets$amt_sum)
    remaining_pct <- quota_structure$payout_pct[nrow(applicable_brackets) + 1]
    payout <- payout + (remaining_amt * remaining_pct)
  }
...
# Test the function
print(calculate_earnings(0.5))  # Example for 50% of the quota
...
# Apply the function to each percentage
earnings <- sapply(percentages, calculate_earnings)

# Create the dataframe with results
results_df <- data.frame(
  percent_quota_earned = percentages,
  payout = earnings
)
...
# Create the plot
ggplot(quota_structure, aes(x = amt_sum, y = payout_sum)) +
  geom_point() +
  geom_line() +
  labs(title = "Amounts Earned vs Quarterly Quota Reached",
...
# Ensure that you have the ggplot2 package installed
install.packages("ggplot2")
library(ggplot2)

# Assuming results_df is your second dataframe with columns percent_quota_earned and payout
# Create the plot for the second dataframe
ggplot(results_df, aes(x = percent_quota_earned * 225000, y = payout)) +
  geom_point() +
  geom_line() +
  labs(
    title = "Payout vs. Dollar Amount Earned",
    x = "Dollar Amount Earned ($)",
    y = "Payout in Dollars ($)"
  ) +
  theme_minimal()
